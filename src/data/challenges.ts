...
public class BlockScope extends LinearOpMode {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Challenge 57 — Broken Motor TeleOp (debugging)
  // ─────────────────────────────────────────────────────────────────────────
  {
    id: 57,
    title: "Broken Motor TeleOp",
    difficulty: "Beginner",
    description:
      "Find and fix multiple bugs in a TeleOp that compiles but misbehaves on the robot — wrong config name, missing loop telemetry, and no stick deadzone.",
    xp: 75,
    estimatedTime: "25 min",
    tags: ["Debugging", "TeleOp", "Motors", "Telemetry", "Gamepad"],
    objectives: [
      "Fix the hardwareMap device name to match the robot configuration (left_motor).",
      "Call telemetry.update() inside the main loop after addData().",
      "Apply a joystick deadzone so the motor stops when the stick is at rest.",
      "Keep the Y-axis negated so forward on the stick drives forward.",
      "Keep all driving logic inside while (opModeIsActive()).",
    ],
    instructions: `This OpMode **compiles** but has several **functional bugs** that will cause crashes or bad driver-station behavior on a real robot.

Your robot configuration names the motor \`"left_motor"\` (all lowercase). The starter code uses a different spelling.

During the match, \`telemetry.addData()\` runs every loop but the Driver Station never refreshes loop data. Init-time telemetry alone is not enough.

The stick value is passed straight to the motor with **no deadzone**. A worn joystick resting at 0.02 will make the motor hum and creep.

**Fix every issue** so the OpMode initializes safely, drives correctly from the left stick, shows live power on the Driver Station, and stops the motor when the stick is centered.`,
    starterCode: `package org.firstinspires.ftc.teamcode;
...
public class BrokenMotorTeleOp extends LinearOpMode {
    ...
    hints: [
      "Robot config names are case-sensitive — change \"Left_Motor\" to \"left_motor\".",
      "telemetry.update() before waitForStart() only runs once — add another update() at the end of the loop.",
      "Add deadzone logic: if Math.abs(gamepad1.left_stick_y) < 0.05, set motorPower to 0 before setPower().",
    ],
    conceptsCovered: [
      "hardwareMap naming",
      "Telemetry refresh in loop",
      "Joystick deadzone",
      "TeleOp debugging",
    ],
  },
