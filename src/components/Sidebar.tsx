...
function NavLink({
  ...
  return (
    ...
    >
      ...
      {badge && (
        <span className="rounded-full bg-amber-500/20 px-2 py-0.5 text-[10px] font-semibold text-amber-400 uppercase tracking-wide">
          {badge}
        </span>
      )}
      {isActive && (
      ...
      >
        <BookOpen
          className={`h-4 w-4 shrink-0 ${
            isDocsActive ? "text-amber-400" : "text-slate-500 group-hover:text-slate-300"
          }`}
        />
      ...
      >
        ...
        <div className="flex h-14 shrink-0 items-center justify-between px-4 border-b border-slate-800/80">
          <div className="flex items-center gap-2.5">
            <div className="flex h-7 w-7 items-center justify-center rounded-md bg-amber-500/20 border border-amber-500/30">
